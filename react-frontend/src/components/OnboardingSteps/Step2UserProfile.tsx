...
const Step2UserProfile: React.FC<StepProps> = ({ data, onUpdate, onNext, onBack }) => {
  ...
  const handleNumberOfDependentsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseInt(e.target.value, 10);
    if (!isNaN(value) && value > 0) {
      onUpdate({ numberOfDependents: value });
      setErrors((prev) => ({ ...prev, numberOfDependents: '' }));
    } else if (e.target.value === '') {
      onUpdate({ numberOfDependents: undefined });
    }
  };
  ...
  const validateAndProceed = () => {
    ...
    if (!data.numberOfDependents || data.numberOfDependents < 1) {
      newErrors.numberOfDependents = 'Please enter the number of dependents';
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        >
          How many dependents do you have? <span className="text-red-500">*</span>
        </label>
        <input
          type="number"
          min="1"
          value={data.numberOfDependents || ''}
          onChange={handleNumberOfDependentsChange}
        ...
        />
        {errors.numberOfDependents && (
          <p className="mt-2 text-sm text-red-500">{errors.numberOfDependents}</p>
        )}
        <p
        ...
        >
          People in your household you're caring for or helping with wellness goals
        </p>
      </div>
...
export default Step2UserProfile;
